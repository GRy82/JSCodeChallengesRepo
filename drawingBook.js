...
// They always turn pages one at a time. When they open the book, page 1 is always on
//  the right side:
...
function getPageFlipCount(pagesInBook, targetPage) {    
    let flipsFromFront = targetPage % 2 === 0 ? targetPage / 2 : Math.floor(targetPage / 2);
    let flipsFromBack = null;
    let pageDiff = pagesInBook - targetPage;
    if(pagesInBook % 2 === 0)
        flipsFromBack = pageDiff % 2 === 0 ? pageDiff / 2 : Math.ceil(pageDiff / 2);
    else
        flipsFromBack = Math.floor(pageDiff / 2);
    return Math.min(flipsFromBack, flipsFromFront);
}